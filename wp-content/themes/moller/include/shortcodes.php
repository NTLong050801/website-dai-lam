<?php
function moller_logo_shortcode( $atts ) {
...
function moller_mainmenu_shortcode( $atts ) {
...
function moller_stickymenu_shortcode( $atts ) {
	$moller_opt = get_option( 'moller_opt' );
	$html = '';
	ob_start(); ?>
		<div class="main-menu-wrapper">
...
function moller_mobilemenu_shortcode( $atts ) {
	$moller_opt = get_option( 'moller_opt' );
	$html = '';
	ob_start(); ?>
		<div class="visible-small mobile-menu"> 
			<div class="mbmenu-toggler"><?php echo esc_html($moller_opt['mobile_menu_label']);?><span class="mbmenu-icon"><i class="fa fa-bars"></i></span></div>
...
function moller_roadcategoriesmenu_shortcode ( $atts ) {
	$atts = shortcode_atts( array(
	...
	<div class="categories-menu-wrapper">
		<div class="categories-menu-inner">
			<div class="categories-menu visible-large <?php echo esc_attr($cat_menu_class); ?>">
				<div class="catemenu-toggler"><?php if(isset($moller_opt['categories_menu_label'])) { echo esc_html($moller_opt['categories_menu_label']); } else { esc_html_e('All Derpartment', 'moller'); } ?></div>
				<div class="menu-inner">
					...
					<div class="morelesscate">
						<span class="morecate"><i class="fa fa-plus"></i><?php if ( isset($moller_opt['categories_more_label']) && $moller_opt['categories_more_label']!='' ) { echo esc_html($moller_opt['categories_more_label']); } else { esc_html_e('More Categories', 'moller'); } ?></span>
						<span class="lesscate"><i class="fa fa-minus"></i><?php if ( isset($moller_opt['categories_less_label']) && $moller_opt['categories_less_label']!='' ) { echo esc_html($moller_opt['categories_less_label']); } else { esc_html_e('Close Menu', 'moller'); } ?></span>
					</div>
				</div>
			</div>
		</div>
...
function moller_roadsocialicons_shortcode( $atts ) {
	$moller_opt = get_option( 'moller_opt' );
	$html = '';
	ob_start();
	if(isset($moller_opt['social_icons'])) {
		echo '<ul class="social-icons">';
		foreach($moller_opt['social_icons'] as $key=>$value ) {
			if($value!=''){
				if($key=='vimeo'){
...
function moller_roadminicart_shortcode( $atts ) {
	$html = '';
	ob_start();
...
function moller_roadwishlist_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'style'  => 'style1',
	), $atts, 'roadwishlist' );
	$html = '';
...
function moller_roadproductssearch_shortcode( $atts ) {
	$html = '';
	ob_start();
  		...
  		<div class="header-search">
	  		<div class="search-without-dropdown">
		  		<div class="categories-container">
		  			<div class="cate-toggler-wrapper"><div class="cate-toggler"><span class="cate-text"><?php esc_html_e('All', 'moller');?></span></div></div>
...
function moller_roadproductssearchdropdown_shortcode( $atts ) {
	$html = '';
	ob_start();
	if( class_exists('WC_Widget_Product_Categories') && class_exists('WC_Widget_Product_Search') ) { ?>
		<div class="header-search">
...
function moller_brands_shortcode( $atts ) {
	global $moller_opt;
	$brand_index = 0;
	if(isset($moller_opt['brand_logos'])) {
		$brandfound = count($moller_opt['brand_logos']);
	}
	$atts = shortcode_atts( array(
		'rows'             => '1',
		'items_1200up'     => '7',
	...
	$html = '';
	if ($atts["items_1200up"]   == '' ) {$atts["items_1200up"]   = 7; }
	if ($atts["items_992_1199"] == '' ) {$atts["items_992_1199"] = 7; }
	if ($atts["items_768_991"]  == '' ) {$atts["items_768_991"]  = 6; }
	if ($atts["items_640_767"]  == '' ) {$atts["items_640_767"]  = 4; }
	if ($atts["items_480_639"]  == '' ) {$atts["items_480_639"]  = 4; }
	if ($atts["items_0_479"]    == '' ) {$atts["items_0_479"]    = 3; }
	...
	if(isset($moller_opt['brand_logos']) && $moller_opt['brand_logos']) {
		$html .= '<div class="brands-carousel roadthemes-slider '.$atts["navigation_style"].' '.$atts["style"].'" data-margin='.$margin.' data-1200up='.$atts["items_1200up"].' data-992-1199='.$atts["items_992_1199"].' data-768-991='.$atts["items_768_991"].' data-640-767='.$atts["items_640_767"].' data-480-639='.$atts["items_480_639"].' data-0-479='.$atts["items_0_479"].' data-navigation='.$navigation.' data-pagination='.$pagination.' data-speed='.$atts["speed"].' data-loop='.$loop.' data-auto='.$auto.'>';
			foreach($moller_opt['brand_logos'] as $brand) {
				if(is_ssl()){
...
function moller_imageslider_shortcode( $atts ) {
	global $moller_opt;
	$image_slider_index = 0;
	if(isset($moller_opt['image_slider'])) {
		$image_slider_found = count($moller_opt['image_slider']);
	}
	$atts = shortcode_atts( array(
		'rows'             => '1',
		'items_1200up'     => '4',
	...
	if(isset($moller_opt['image_slider']) && $moller_opt['image_slider']) {
		$html .= '<div class="image-slider roadthemes-slider '.$atts["navigation_style"].' '.$atts["style"].'" data-margin='.$margin.' data-1200up='.$atts["items_1200up"].' data-992-1199='.$atts["items_992_1199"].' data-768-991='.$atts["items_768_991"].' data-640-767='.$atts["items_640_767"].' data-480-639='.$atts["items_480_639"].' data-0-479='.$atts["items_0_479"].' data-navigation='.$navigation.' data-pagination='.$pagination.' data-speed='.$atts["speed"].' data-loop='.$loop.' data-auto='.$auto.'>';
			foreach($moller_opt['image_slider'] as $image) {
				if(is_ssl()){
				...
				$html .= '<div class="image-slider-inner">';
					...
					$html .= '<div class="title">';
						$html .= '<h3>' . esc_html($image['title']) . '</h3>';
						$html .= '<a href="'.$image['url'].'" title="'.esc_attr($image['title']).'">';
							$html .= esc_html__('Discover Now', 'moller');
						$html .= '</a>';
					$html .= '</div>';
				$html .= '</div>';
...
function moller_counter_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'image'  => '',
		'number' => '100',
		'text'   => 'text',
...
function moller_popular_categories_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'category' => '',
		'image'    => ''
...
function moller_latestposts_shortcode( $atts ) {
	global $moller_opt;
	$post_index = 0;
	$atts = shortcode_atts( array(
		'posts_per_page'   => 10,
	...
	if($atts['image']=='wide'){
		$imagesize = 'moller-post-thumbwide';
	} else {
		$imagesize = 'moller-post-thumb';
	}
	if($atts['category']=='0'){
		$atts['category'] = '';
	}
					...
					$html.='<div class="post-info">';						
						$html.='<div class="post-meta">';
							$html.='<div class="post-author">';
								$html.= sprintf( wp_kses(__( '%s', 'moller' ), array('a'=>array('href'=>array()))), __('By', 'moller').' <a href="'.$author_url.'">'.$author_name.' </a>' );
							$html.='</div>';
							if(has_category('',$post->ID)) {
								$html.='<div class="post-category">';
									$html.= '<span>' . esc_html('in', 'moller') . '</span>';
									$html.= ' ';
						...
						$html.='<div class="post-excerpt">';
							$html.= Moller_Features::moller_excerpt_by_id($post, $length = $atts['length']);
						$html.='</div>';
						$html.='<div class="btn-readmore-date">';
							$html.='<a class="readmore" href="'.get_the_permalink($post->ID).'">'.'<span>' .esc_html($moller_opt['readmore_text']). '</span>'.'</a>';
							$html.='<div class="">';
								$html.='<div class="post-date">' . esc_html('By ', 'moller') . $author_name . ', ' . get_the_date('F d, Y', $post->ID) . '</div>';
							$html.='</div>';
						$html.='</div>';
...
function moller_testimonials_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'limit'            => '10',
		'orderby'          => 'menu_order',
		'order'            => 'DESC',
...
function moller_magnifier_options($att) {  
	...
	wp_enqueue_script('moller-magnifier', get_template_directory_uri() . '/js/product-magnifier-var.js');
	wp_localize_script('moller-magnifier', 'moller_magnifier_vars', array(
...
function moller_heading_title_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'heading_title'      => 'Title',
		'sub_heading_title'  => '',
		'style'              => 'style1',
...
function moller_countdown_shortcode( $atts ) {
	$atts = shortcode_atts( array(
		'countdown_day'    => '1',
		'countdown_month'  => '1',
		'countdown_year'   => '2020',
...
function moller_login_logout_shortcode( $atts ){
	$atts = shortcode_atts( array(
		'txt_wellcome'    => 'Hello',
		'txt_login'    => 'Login',
		'txt_logout'    => 'Logout',
