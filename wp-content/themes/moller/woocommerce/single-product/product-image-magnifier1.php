...
<div class="images<?php if($is_IE): ?> ie<?php endif ?> <?php echo esc_attr($imageclass); ?>">
	...
	if ( has_post_thumbnail () ) {
        ...
        echo apply_filters ( 'woocommerce_single_product_image_html', sprintf ( '<a href="%s" class="yith_magnifier_zoom woocommerce-main-image %s" title="%s">%s</a>', $magnifier_url,$extra_classes, $image_title, $image ), $post->ID );

    } else {
        echo apply_filters ( 'woocommerce_single_product_image_html', sprintf ( '<a href="%s" class="yith_magnifier_zoom woocommerce-main-image %s"><img src="%s" alt="Placeholder" /></a>', $placeholder, $extra_classes, $placeholder ), $post->ID );
    }
	?>
