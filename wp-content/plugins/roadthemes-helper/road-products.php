...
}	// require_once( $path_to_wp.'wp-content/plugins/woocommerce/includes/shortcodes/class-wc-shortcode-products.php');	// require_once( $path_to_wp.'wp-content/plugins/js_composer/include/classes/vendors/plugins/class-vc-vendor-woocommerce.php');	
class RoadProducts {
	...
	public function road_vc_shortcodes() {
		...
		$order_by_values = array(
			...
			__( 'Date', 'moller' )          => 'date',
			__( 'ID', 'moller' )            => 'ID',
			__( 'Author', 'moller' )        => 'author',
			__( 'Title', 'moller' )         => 'title',
			__( 'Modified', 'moller' )      => 'modified',
			__( 'Random', 'moller' )        => 'rand',
			__( 'Comment count', 'moller' ) => 'comment_count',
			__( 'Menu order', 'moller' )    => 'menu_order',
		);
		$order_way_values = array(
			'',
			__( 'Descending', 'moller' ) => 'DESC',
			__( 'Ascending', 'moller' )  => 'ASC',
		); 
	//Sale products Roadthemes
	vc_map( array(
		'name'        => __( 'RT Sale products', 'moller' ),
		'base'        => 'sale_products_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'moller' ),
		'description' => __( 'List all products on sale', 'moller' ),
		'params'      => array_merge(
			array(
				array(
					'type'        => 'textfield',
					'heading'     => esc_html__( 'Per page', 'moller' ),
					'value'       => 12,
					'save_always' => true,
					'param_name'  => 'per_page',
					'description' => esc_html__( 'How much items per page to show', 'moller' ),
				),
					array(
					'type'        => 'textfield',
					'heading'     => __( 'Columns', 'moller' ),
					'value'       => 4,
					'save_always' => true,
					'param_name'  => 'columns',
					'description' => __( 'How much columns grid', 'moller' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Order by', 'moller' ),
					'param_name'  => 'orderby',
					'value'       => $order_by_values,
					'save_always' => true,
					'description' => sprintf( __( 'Select how to sort retrieved products. More at %s.', 'moller' ), '<a href="http://codex.wordpress.org/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">WordPress codex page</a>' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Sort order', 'moller' ),
					'param_name'  => 'order',
					'value'       => $order_way_values,
					'save_always' => true,
					'description' => sprintf( __( 'Designates the ascending or descending order. More at %s.', 'moller' ), '<a href="http://codex.wordpress.org/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">WordPress codex page</a>' ),
				), 
	...
	vc_map( array(
		'name'        => __( 'RT Featured products', 'moller' ),
		'base'        => 'featured_products_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'moller' ),
		'description' => __( 'Display products set as "featured"', 'moller' ),
		'params'      => array_merge(
		array(
			array(
				'type'        => 'textfield',
	...
	vc_map( array(
	'name'        => __( 'RT Recent products', 'moller' ),
	'base'        => 'recent_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'moller' ),
	'description' => __( 'Lists recent products', 'moller' ),
	'params'      => array_merge( 
		array(
			array(
				'type'        => 'textfield',
	...
	vc_map( array(
	'name' => __( 'Top Rated Products', 'moller' ),
	'base' => 'top_rated_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category' => __( 'Theme', 'moller' ),
	'description' => __( 'List all top rated products', 'moller' ),
	'params'      => array_merge( 
		array(
			array(
				'type' => 'textfield',
				'heading' => __( 'Per page', 'moller' ),
				'value' => 12,
				'param_name' => 'per_page',
				'save_always' => true,
				'description' => __( 'How much items per page to show', 'moller' ),
			),
			array(
				'type' => 'textfield',
				'heading' => __( 'Columns', 'moller' ),
	...
	vc_map( array(
	'name'        => __( 'RT Bestselling products', 'moller' ),
	'base'        => 'bestselling_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'moller' ),
	'description' => __( 'List best selling products on sale', 'moller' ),
	'params'      => array_merge( 
		array(
				array(
					'type'        => 'textfield',
	...
	//products Roadthemes
	vc_map( array(
	'name'        => __( 'RT Products', 'moller' ),
	'base'        => 'list_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'moller' ),
	'description' => __( 'Show multiple products by ID or SKU.', 'moller' ),
	'params'      => array_merge(
		array(
			array(
				'type'        => 'textfield',
			...
			array(
				...
				'save_always' => true,
				'description' => __( 'Enter List of Products', 'moller' ),
			),
			array(
				'type'       => 'hidden',
				'param_name' => 'skus',
	...
	vc_map( array(
	'name'        => __( 'RT Product categories', 'moller' ),
	'base'        => 'product_categories_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'moller' ),
	'description' => __( 'Display product categories loop', 'moller' ),
	'params'      => array_merge( 
			array(
				array(
					'type'        => 'textfield',
					'heading'     => __( 'Number', 'moller' ),
					'param_name'  => 'number',
					'description' => __( 'The `number` field is used to display the number of products.', 'moller' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Order by', 'moller' ),
				...
				array(
					...
					'description' => __( 'Hide empty', 'moller' ),
				),
				array(
					'type'       => 'autocomplete',
					'heading'    => __( 'Categories', 'moller' ),
					'param_name' => 'ids',
					'settings'   => array(
						'multiple' => true,
						'sortable' => true,
					...
					'save_always' => true,
					'description' => __( 'List of product categories', 'moller' ),
				), 
			),road_get_slider_setting()
		)
	)  );
		...
		vc_map( array(
		'name'        => __( 'RT Product category', 'moller' ),
		'base'        => 'product_category_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'moller' ),
		'description' => __( 'Show multiple products in a category', 'moller' ),
		'params'      => array_merge(
			array(
				array(
					'type'        => 'textfield',
				...
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Category', 'moller' ),
					'value'       => $product_categories_dropdown,
					'param_name'  => 'category',
					'save_always' => true,
					'description' => __( 'Product category list', 'moller' ),
				),
			),road_get_slider_setting()
		)
	)  );
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $product_object ) ) {
				...
				if ( ! empty( $product_sku ) ) {
					$product_sku_display = ' - ' . __( 'Sku', 'moller' ) . ': ' . $product_sku;
				}
				$product_title_display = '';
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . __( 'Title', 'moller' ) . ': ' . $product_title;
				}
				$product_id_display = __( 'Id', 'moller' ) . ': ' . $product_id;
				$data = array();
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = __( 'Id', 'moller' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . __( 'Title', 'moller' ) . ': ' . $value['title'] : '' ) . ( ( strlen( $value['sku'] ) > 0 ) ? ' - ' . __( 'Sku', 'moller' ) . ': ' . $value['sku'] : '' );
				$results[] = $data;
			}
		}
		return $results;
	...
	public function productCategoryCategoryAutocompleteSuggester( $query, $slug = false ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = __( 'Id', 'moller' ) . ': ' . $value['id'] . ( ( strlen( $value['name'] ) > 0 ) ? ' - ' . __( 'Name', 'moller' ) . ': ' . $value['name'] : '' ) . ( ( strlen( $value['slug'] ) > 0 ) ? ' - ' . __( 'Slug', 'moller' ) . ': ' . $value['slug'] : '' );
				$result[] = $data;
			}
		}
		return $result;
	...
	protected function productCategoryTermOutput( $term ) {
		...
		$term_slug_display = '';
		if ( ! empty( $term_slug ) ) {
			$term_slug_display = ' - ' . __( 'Sku', 'moller' ) . ': ' . $term_slug;
		}
		$term_title_display = '';
		if ( ! empty( $term_title ) ) {
			$term_title_display = ' - ' . __( 'Title', 'moller' ) . ': ' . $term_title;
		}
		$term_id_display = __( 'Id', 'moller' ) . ': ' . $term_id;
		$data = array();
		$data['value'] = $term_id;
	...
	public static function getProductsFieldsList() {
		return array(
			__( 'SKU', 'moller' )               => 'sku',
			__( 'ID', 'moller' )                => 'id',
			__( 'Price', 'moller' )             => 'price',
			__( 'Regular Price', 'moller' )     => 'regular_price',
			__( 'Sale Price', 'moller' )        => 'sale_price',
			__( 'Price html', 'moller' )        => 'price_html',
			__( 'Reviews count', 'moller' )     => 'reviews_count',
			__( 'Short description', 'moller' ) => 'short_description',
			__( 'Dimensions', 'moller' )        => 'dimensions',
			__( 'Rating count', 'moller' )      => 'rating_count',
			__( 'Weight', 'moller' )            => 'weight',
			__( 'Is on sale', 'moller' )        => 'on_sale',
			__( 'Custom field', 'moller' )      => '_custom_',
		);
	}
	...
	public static function getOrderFieldsList() {
		return array(
			__( 'ID', 'moller' )                       => 'id',
			__( 'Order number', 'moller' )             => 'order_number',
			__( 'Currency', 'moller' )                 => 'order_currency',
			__( 'Total', 'moller' )                    => 'total',
			__( 'Status', 'moller' )                   => 'status',
			__( 'Payment method', 'moller' )           => 'payment_method',
			__( 'Billing address city', 'moller' )     => 'billing_address_city',
			__( 'Billing address country', 'moller' )  => 'billing_address_country',
			__( 'Shipping address city', 'moller' )    => 'shipping_address_city',
			__( 'Shipping address country', 'moller' ) => 'shipping_address_country',
			__( 'Customer Note', 'moller' )            => 'customer_note',
			__( 'Customer API', 'moller' )             => 'customer_api',
			__( 'Custom field', 'moller' )             => '_custom_',
		);
	}
